...
//Creates the markers around gunstores.
{
	if (!isPlayer _x && {["GunStore", vehicleVarName _x] call fn_startsWith}) then
	{
		...
		*/
		...
		_markerName setMarkerColorLocal _col_empty;
		_markerName setMarkerSizeLocal [1,1];
		_markerName setMarkerTextLocal "ARMURERIE";
		// _markerName setMarkerAlphaLocal 0.5;
...
_setStatus =
{
	...
	if (["A3W_gunStoreIntruderWarning"] call isConfigOn) then
	{
		if((_this select 2) && ((_this select 1) in ["ENEMY", "MIXED"])) then {
			hintSilent parseText format ["<t size='2' color='#ff0000'>%1</t><br/><br/>%2.","ATTENTION!","Un joueur ennemi est entré dans la zone !"];
		};
	};
