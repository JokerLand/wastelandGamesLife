...
#define ERR_IN_VEHICLE "Refueling échoué! Vous ne pouvez pas faire ça depuis un véhicule"
#define ERR_TOO_FAR_AWAY "Refueling échoué! Vous êtes parti trop loin du véhicule"
#define ERR_CANCELLED "Refueling annulé!"
...
_checks = {
	...
	switch (true) do {
		...
		default {
			_text = format["Refuel du véhicule %1%2 terminé", round(100 * _progress), "%"];
			_failed = false;
	    };
	};
	[_failed, _text];
...
if (_success) then {
	...
	["Refuel terminé !", 5] call mf_notify_client;
};
false;
