...
[MF_ITEMS_WARCHEST, "Coffre", mf_items_warchest_deploy, "Land_SuitCase_F", _icon, 1] call mf_inventory_create;

private ["_condition", "_action"];
_condition = "'' == [] call mf_items_warchest_can_access;";
_action = [format ["<img image='%1'/> Accéder au coffre", _icon], mf_items_warchest_access, nil, 2, true, false, "", _condition];
["warchest-access", _action] call mf_player_actions_set;

_condition = "'' == [] call mf_items_warchest_can_pack;";
_action = [format ["<img image='%1'/> Installer le coffre", _icon], mf_items_warchest_pack, nil, 1, true, false, "", _condition];
["warchest-pack", _action] call mf_player_actions_set;

_condition = "'' == [] call mf_items_warchest_can_hack;";
_action = [format ["<img image='%1'/> Voler le coffre", _icon], mf_items_warchest_hack, nil, 2, true, false, "", _condition];
